...
def sync_prices(payload: SyncRequest, db: Session = Depends(get_db)):
    ...
    if payload.backfill_days and payload.backfill_days > 0:
        start = (datetime.utcnow() - timedelta(days=payload.backfill_days)).date()

    for sym in payload.symbols:
        symbol = sym.strip().upper()
        if not symbol:
            continue
        inst = db.query(models.Instrument).filter(models.Instrument.symbol == symbol).first()
        if not inst:
        ...
        for b in bars:
            # upsert by (instrument_id, ts)
            exists = db.query(models.Price).filter(models.Price.instrument_id == inst.id, models.Price.ts == b["ts"]).first()
            if exists:
                continue
            p = models.Price(
